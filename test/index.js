...
describe("node-async", () => {
	...
	// it("async stream: distribute", async () => {
	// 	const resultMap = {};
	// 	for (const stream of AsyncStream.from(createMockGenerator(0, 10, 1)).distribute(5)) {
	// 		console.log("hoge");
	// 		for await (const value of stream.map(v => v.toString())) {
	// 			if (value in resultMap) {
	// 				resultMap[value]++;
	// 			} else {
	// 				resultMap[value] = 1;
	// 			}
	...
	// 	for (const value of range(10).map(v => v.toString())) {
	// 		except(resultMap[value]).is.equal(5);
	// 	}
	// });
